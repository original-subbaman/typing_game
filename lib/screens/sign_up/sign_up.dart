...
class _SignUpScreenState extends ConsumerState<SignUpScreen> {
  ...
  void showCreateUserException(String status) {
    if (status == SUCCESS) {
      Navigator.popUntil(context, ModalRoute.withName(Wrapper.kLandingPage));
    } else {
      showSnackBar(msg: "Error creating user. Try again.", context: context);
      // If creating user in Firestore fails, delete the user from FirebaseAuth
      // This ensures that we don't have FirebaseAuth account that does not have a
      // corresponding Firestore entry.
      ref.read(authRepositoryProvider).deleteCurrentUser();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final dbProvider = ref.read(firestoreProvider);
    return WillPopScope(
      onWillPop: () async {
          ...
          floatingActionButton: SizedBox(
            ...
            child: FittedBox(
              child: FloatingActionButton(
                ...
                onPressed: () async {
                  dismissKeyboard();
                  String signUpStatus = await authProvider.signUpUser(
                      email: _emailController.text.toString(),
                      password: _passwordController.text.toString());

                  if (signUpStatus != "success") {
                    showUserSignUpException(signUpStatus);
                  } else {
                    final newPlayer = createNewPlayer();
                    String? createUserEntryStatus =
                        await dbProvider.createNewPlayer(
                            MyFirebaseAuth.currentUserId, newPlayer);
                    showCreateUserException(createUserEntryStatus!);
                  }
                },
              ),
            ),
